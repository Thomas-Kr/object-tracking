...
class Tracker:
    def __init__(self, frame, tracker=cv2.TrackerCSRT_create()):
        self.track_method = tracker
        self.bbox, self.frame = self._set_tracker(frame)
        self.template = self._create_template()

    def _set_tracker(self, frame) -> cv2.numpy.ndarray:
    ...
    def track(self, frame, threshold=0.9):
